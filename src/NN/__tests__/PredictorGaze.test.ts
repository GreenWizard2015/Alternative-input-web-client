/**
 * PredictorGaze Integration Test
 *
 * Tests the complete PredictorGaze pipeline:
 * - Load test data from PredictorGaze.zip
 * - Run inference through PredictorGaze
 * - Validate outputs match Python reference implementation
 */

import { PredictorGaze } from '../PredictorGaze';
import { assertTensorsClose } from './testHelpers';
import { tensorsFromZip, disposeAll } from '../utils/tensorflow';
import { cleanupTensorFlow } from './setup';

describe('PredictorGaze - Complete Pipeline Integration', () => {
  beforeEach(() => {
    cleanupTensorFlow();
  });

  afterEach(() => {
    cleanupTensorFlow();
  });
  test('should run inference with ZIP data and validate outputs', async () => {
    // Initialize PredictorGaze with correct parameters based on Python ground truth
    // Python PredictorGaze uses default constructor
    const model = new PredictorGaze({
      name: 'PredictorGaze_layer',
    });

    const path = require('path');
    const zipPath = path.join(__dirname, '../__tests__/data/PredictorGaze.zip');

    // Load tensors from ZIP using utility function
    const tensors = await tensorsFromZip(zipPath);
    model.build(tensors.get('inputs/input')!.shape);
    await model.loadWeights(tensors);

    const output = model.apply(tensors.get('inputs/input')!, false);
    assertTensorsClose(output, tensors.get('outputs/predictions')!);

    // Clean up
    tensors.dispose();
    output.dispose();
    model.dispose();
  });
});
